...
const EMPTY: any = {investment: 0, investor: 0, month: 0, year: 0, base_amount: 0, dividend_rate: 0, dividend_amount: 0, status: ''}
...
export default function Page() {
  ...
  return (
    <AppShell>
      <PageHeader title="Dividends" subtitle="Monthly dividend records" onAdd={openAdd} addLabel="New Dividend" />
      <div className="card">
        {loading ? <Spinner /> : items.length === 0 ? <Empty /> : (
          <div className="table-wrap">
            <table>
              <thead><tr><th>investor</th><th>investment</th><th>month</th><th>year</th><th>dividend_rate</th><th>dividend_amount</th><th>status</th><th>Actions</th></tr></thead>
              <tbody>{items.map(item => (
                <tr key={item.id}>
                  <td>{item.investor ?? '—'}</td><td>{item.investment ?? '—'}</td><td>{item.month ?? '—'}</td><td>{item.year ?? '—'}</td><td>{item.dividend_rate ?? '—'}</td><td>{item.dividend_amount ?? '—'}</td><td><Badge status={String(item.status)} /></td>
                  <td>
      ...
      {(modal === 'add' || modal === 'edit') && (
        <Modal title={modal === 'add' ? 'New Dividends' : 'Edit Dividends'} onClose={() => setModal(null)} size="lg">
          <div className="modal-body">
            <div className="form-grid">
              <div><label className="label">Investment ID *</label><input className="input" type="number" value={form.investment||''} onChange={f('investment')} /></div>
              ...
              <div><label className="label">Base Amount</label><input className="input" type="number" value={form.base_amount||''} onChange={f('base_amount')} /></div>
              <div><label className="label">Rate (%)</label><input className="input" type="number" value={form.dividend_rate||''} onChange={f('dividend_rate')} /></div>
              <div><label className="label">Dividend Amount</label><input className="input" type="number" value={form.dividend_amount||''} onChange={f('dividend_amount')} /></div>
              <div><label className="label">Status</label><select className="input" value={form.status||''} onChange={f('status')}><option key='generated' value='generated'>generated</option><option key='paid' value='paid'>paid</option><option key='pending_withdrawal' value='pending_withdrawal'>pending withdrawal</option></select></div>
            </div>
          </div>
      ...
      {modal === 'view' && selected && (
        <Modal title="Dividends Details" onClose={() => setModal(null)} size="lg">
          <div className="modal-body">
            <div className="form-grid">
